...
from wtforms.fields import DecimalField, SelectFieldBase, StringField
...
class ValidTime(object):
    def __init__(self, seconds=True, message=None):
        self.format = "%H:%M"
        if seconds:
            self.format += ":%S"
        if not message:
            message = "You must provide a time in the format {}".format(self.format)
        self.message = message

    def __call__(self, form, field):
        t = field.data and len(field.data) or ""
        try:
            time.strptime(t, self.format)
        except ValueError:
...
class TimeField(DecimalField):
    def _value(self):
        ...
        return strfdelta(self.data, "{hours}:{minutes:02d}")

    def process_formdata(self, valuelist):
        if len(valuelist) == 1 and valuelist[0]:
            t = time.strptime(valuelist[0], "%H:%M")
            self.data = timedelta(hours=t.tm_hour, minutes=t.tm_min)
        else:
            self.data = []
...
class PriceField(DecimalField):
    def _value(self):
        if not self.data:
            return ""
        return "${:,.2f}".format(self.data / 100)

    def process_formdata(self, valuelist):
        if len(valuelist) == 1:
            self.data = [int(float(valuelist[0].strip("$").replace(",", "")) * 100)]
        else:
            self.data = []


class QueryStringField(StringField):
    def __init__(self, *args, attribute=None, **kwargs):
        super(QueryStringField, self).__init__(*args, **kwargs)
...
class AutocompleteField(QuerySelectField):
    pass
